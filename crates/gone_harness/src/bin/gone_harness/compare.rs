...
fn compare_impl(args: &[String], render_check: bool) -> Result<i32, RunnerError> {
    ...
    let run_a = run_scenario(&root, &scenario_path, &scenario, &out_root, render_check, DEFAULT_TIMEOUT)?;
    let run_b = run_scenario(&root, &scenario_path, &scenario, &out_root, render_check, DEFAULT_TIMEOUT)?;
